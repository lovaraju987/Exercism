...
def reactor_efficiency(voltage, current, theoretical_max_power):
    ...
    return (
        "green" if efficiency_percentage >= 80
        else "orange" if efficiency_percentage >= 60
        else "red" if efficiency_percentage >= 30
        else "black"
    )
...
def fail_safe(temperature, neutrons_produced_per_second, threshold):
    ...
    """
    overall_netrons_emitted = temperature * neutrons_produced_per_second
    if overall_netrons_emitted < threshold * (90/100):
        return "LOW"
    elif overall_netrons_emitted < (threshold * (10/100)) + threshold:
        return "NORMAL"
    else:
        return "DANGER"
